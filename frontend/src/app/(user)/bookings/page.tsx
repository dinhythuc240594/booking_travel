      ...
      <main className="flex-grow pt-28 sm:pt-36 pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-10 max-w-3xl">
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 text-xs font-bold uppercase tracking-wider mb-3">
              <History className="w-3.5 h-3.5" /> Lịch sử hoạt động
            </span>
            <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight mb-3">
              Hành Trình Đã Đặt
            </h1>
            <p className="text-sm text-zinc-500 dark:text-zinc-400 leading-relaxed">
              Theo dõi lịch trình của bạn, xem lại thông tin đặt vé, trạng thái thanh toán và thực hiện yêu cầu hủy tour nếu thay đổi kế hoạch di chuyển.
            </p>
          </div>
          ...
          {myBookings.length === 0 ? (
            /* Trạng thái trống */
            <div className="text-center py-20 bg-white dark:bg-zinc-900 border border-zinc-200/60 dark:border-zinc-800/60 rounded-3xl p-8 shadow-sm">
              <div className="w-20 h-20 bg-gradient-to-tr from-cyan-400/10 to-blue-500/10 text-cyan-500 rounded-3xl flex items-center justify-center mx-auto mb-6">
            ...
            <div className="space-y-6">
              ...
              {/* DESKTOP TABLE VIEW */}
              <div className="hidden md:block overflow-hidden bg-white dark:bg-zinc-900 border border-zinc-200/50 dark:border-zinc-800/50 rounded-3xl shadow-sm">
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="border-b border-zinc-100 dark:border-zinc-800 text-[10px] font-bold uppercase tracking-wider text-zinc-400 dark:text-zinc-500 bg-zinc-50/50 dark:bg-zinc-950/20">
                        <th className="py-4.5 px-6">Tour</th>
                        <th className="py-4.5 px-4">Khởi hành</th>
                        <th className="py-4.5 px-4">Hành khách</th>
                        <th className="py-4.5 px-4">Tổng chi phí</th>
                        <th className="py-4.5 px-4">Trạng thái đặt</th>
                        <th className="py-4.5 px-4">Thanh toán</th>
                        <th className="py-4.5 px-6 text-right">Thao tác</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-zinc-100 dark:divide-zinc-855 text-sm">
                      {myBookings.map((booking) => (
                        <tr key={booking.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-950/10 transition-colors">
                          ...
                          <td className="py-5 px-6 max-w-xs">
                            <div className="flex items-center gap-4">
                              <div className="w-14 h-14 rounded-xl overflow-hidden bg-zinc-100 dark:bg-zinc-800 flex-shrink-0">
                                ...
                                <img
                                  src={booking.tourImage || "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=300&auto=format&fit=crop&q=80"}
                              ...
                              <div className="min-w-0">
                                <Link 
                                  ...
                                  className="font-bold text-zinc-950 dark:text-zinc-100 hover:text-cyan-500 dark:hover:text-cyan-400 transition-colors line-clamp-1"
                                >
                                  {booking.tourTitle}
                                </Link>
                          ...
                          {/* Date */}
                          <td className="py-5 px-4 font-semibold text-zinc-700 dark:text-zinc-300">
                          ...
                          <td className="py-5 px-4">
                            <div className="flex flex-col gap-0.5">
                              <span className="font-bold text-zinc-950 dark:text-zinc-100 flex items-center gap-1">
                                <Users className="w-3.5 h-3.5 text-zinc-400" /> {booking.adults + booking.children} khách
                              </span>
                              <span className="text-[11px] text-zinc-400 pl-4.5">
                                {booking.adults} NL {booking.children > 0 && `· ${booking.children} TE`}
                              </span>
                            </div>
                          </td>
                          {/* Total Price */}
                          <td className="py-5 px-4 font-extrabold text-cyan-500 dark:text-cyan-400 text-base">
                          ...
                          <td className="py-5 px-6 text-right">
                            {(booking.status === BookingStatus.PENDING || booking.status === BookingStatus.CONFIRMED) ? (
                              <button
                                ...
                                className="px-4 py-2 text-xs font-bold text-red-500 hover:text-white border border-red-500/30 hover:border-red-600 bg-transparent hover:bg-red-650 rounded-xl transition-all cursor-pointer"
                              >
                                Hủy tour
                              </button>
                            ) : (
              ...
              <div className="block md:hidden space-y-4">
                {myBookings.map((booking) => (
                  <div 
                    key={booking.id}
                    className="bg-white dark:bg-zinc-900 border border-zinc-200/70 dark:border-zinc-800/70 rounded-2xl p-5 shadow-sm space-y-4"
                  >
                    ...
                    <div className="flex gap-4">
                      ...
                      <div className="min-w-0 flex-1">
                        <Link 
                          href={`/tours/${booking.tourId}`}
                          className="font-bold text-zinc-950 dark:text-zinc-100 hover:text-cyan-500 dark:hover:text-cyan-400 transition-colors line-clamp-1 block text-sm"
                        >
                          {booking.tourTitle}
                        </Link>
                        ...
                        <span className="text-xs text-zinc-600 dark:text-zinc-400 font-semibold mt-1 block">
                    ...
                    {(booking.status === BookingStatus.PENDING || booking.status === BookingStatus.CONFIRMED) && (
                      <div className="pt-2">
                        <button
                          onClick={() => handleCancelClick(booking)}
                          className="w-full py-2.5 text-xs font-bold text-red-500 hover:text-white border border-red-500/20 hover:border-red-600 bg-red-500/5 hover:bg-red-650 rounded-xl transition-all cursor-pointer"
                        >
                          Hủy đặt tour này
                        </button>
                      </div>
      ...
      {confirmCancelBooking && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
          ...
          <div className="relative bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 max-w-md w-full rounded-3xl p-6 sm:p-8 shadow-2xl animate-scale-up text-center z-10">
            ...
            {/* Tour Info Preview */}
            <div className="bg-zinc-50 dark:bg-zinc-950 border border-zinc-200/50 dark:border-zinc-850/50 rounded-2xl p-4 text-left text-xs mb-6 space-y-1.5 text-zinc-600 dark:text-zinc-300">
            ...
            <div className="flex gap-3">
              <button
                ...
                onClick={() => setConfirmCancelBooking(null)}
                className="flex-1 py-3 border border-zinc-200 dark:border-zinc-855 hover:bg-zinc-50 dark:hover:bg-zinc-800 text-zinc-700 dark:text-zinc-300 font-bold rounded-xl text-sm transition-all cursor-pointer bg-transparent disabled:opacity-40"
              >
                Quay lại
              </button>
              <button
