...
1. **Quản lý Giỏ hàng/Combo Booking (`AbstractBookingItem`):** Tính tổng giá tiền (`total_price`) cho các combo bao gồm cả Tour (tính theo số người lớn/trẻ em) và Hotel (tính theo số đêm phòng) một cách đồng bộ.
...
```text
                 ...
                 .                                       .                    |
  +------------------------------+       +------------------------------+     |
  |      HotelsBookingItem       |       |       TourBookingItem        |     |
  +------------------------------+       +------------------------------+     |
  | - hotels : Hotels            |       | - tour : Tour                |     |
  | - nights : int               |       | - persons : int              |     |
  +------------------------------+       +------------------------------+     |
  | + get_total_price() : float  |       | + get_total_price() : float  |     |
  | + show_details(indent) : str |       | + show_details(indent) : str |     |
...
2. **Leaf 1 (`HotelsBookingItem`):**
   - Định nghĩa trong [hotel.py](file:///d:/python/booking_travel/src/composite/hotel.py).
   - Đại diện cho dịch vụ lưu trú. Công thức tính giá: `giá khách sạn một đêm * số đêm`.
3. **Leaf 2 (`TourBookingItem`):**
   - Định nghĩa trong [tour.py](file:///d:/python/booking_travel/src/composite/tour.py).
   - Đại diện cho dịch vụ Tour du lịch. Công thức tính giá phức tạp hơn: `(giá người lớn * số người) + (giá trẻ em * số người)`.
4. **Composite (`BookingPackage`):**
   - Định nghĩa trong [booking.py](file:///d:/python/booking_travel/src/composite/booking.py).
   - Đại diện cho một "gói combo" chứa danh sách nhiều `AbstractBookingItem` bên trong (có thể là khách sạn, tour lẻ hoặc thậm chí là một gói combo nhỏ khác).
...
Trong tệp tin [booking_service.py](file:///d:/python/booking_travel/src/service/booking_service.py#L24-L32), khi khách hàng chọn mua một combo du lịch gồm cả khách sạn và tour, hệ thống thực hiện gom các item này vào một `BookingPackage`:
```python
...
# Tính tổng giá tiền cực kỳ dễ dàng mà không cần phân biệt là tour hay khách sạn
...
1. **Bước 1 (Composite):** Client gom các thông tin dịch vụ đơn lẻ (Tour, Hotel) của người dùng thành một đối tượng phức hợp `BookingPackage`. Phương thức `combo.get_total_price()` được gọi để tự động tính tổng tiền của cả gói mà không cần quan tâm chi tiết cách tính giá của từng thành phần con.
...
* **Ưu điểm:**
  - **Dễ dàng quản lý cấu trúc cây:** Giúp code biểu diễn các thực thể lồng nhau (Combo gồm Tour và Khách sạn, Thư mục cài đặt gồm các khóa cấu hình con) một cách rõ ràng.
  - **Tính đa hình cao:** Client không cần viết các câu lệnh điều kiện `if-else` phức tạp để phân biệt xem mình đang xử lý một Tour đơn lẻ hay một Khách sạn hay một gói dịch vụ tổng hợp. Chỉ cần gọi phương thức chung.
